      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsListHtml = details.participants && details.participants.length > 0
          ? `<ul class="participants-list">${details.participants.map(p => `
              <li class="participant-item">
                ...
                <button class="delete-btn" data-activity="${name}" data-email="${p}" title="Unregister student">🗑️</button>
              </li>`).join('')}</ul>`
        ...
        activityCard.innerHTML = `
          ...
          <div class="participants-section">
            <p><strong>Participants (${details.participants ? details.participants.length : 0}):</strong></p>
